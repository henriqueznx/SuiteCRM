...
class SugarController{
	...
	public static $action_case_file = array(
		'editview' => 'EditView', 'detailview' => 'DetailView', 'listview' => 'ListView'
	);
	/**
	...
	public $process_tasks = array(
		'blockFileAccess', 'handleEntryPoint', 'callLegacyCode', 'remapAction', 'handle_action', 'handleActionMaps',
	);
	/**
	...
	protected $tasks = array(
		'pre_action', 'do_action', 'post_action'
	);
	/**
		...
		if (!empty($_REQUEST['id'])) {
			...
			if (!empty($_REQUEST['configure']) && $_REQUEST['configure']) { // save settings
				...
				return 'result = ' . $json->encode((array(
					'header' => $dashlet->title . ' : ' . $mod_strings['LBL_OPTIONS'], 'body' => $dashlet->displayOptions()
				)));
...
*/
	...
	private function do_action() {
		...
		$function = strtolower($this->do_action);
		if ($this->hasFunction($function)) {
