...
EOQ;
    ...
    {
		$script = 'Alerts.prototype.add(' . json_encode($type) .',' . json_encode($name). ',' . json_encode($subtitle). ','. json_encode(str_replace(array("\r", "\n"), array('','<br>'),$description)) . ',' . $countdown . ','.json_encode($redirect).');' . "\n";
        $this->script .= $script;
	}
    ...
    function addActivities(){
		...
		$dateTimeNow = $timedate->getNow()->modify("-1 days")->asDb();
